...
TargetStates = {"victim":[],"obstacle":[],"fire":[]}
...
def convertToHighLevel(obs, n_obs, actions,oldStateNumberStr):
    ...
    if numberOfAgents == 3:
        if (newStateNumber[0] == True) or (newStateNumber[3] == True) or (newStateNumber[6] == True):
            if str(newStateNumber).replace(" ","") not in TargetStates["fire"]:
                TargetStates["fire"].append(str(newStateNumber).replace(" ",""))

    #Obs Complete
    if numberOfAgents == 3:
        if (newStateNumber[1] == True) or (newStateNumber[4] == True) or (newStateNumber[7] == True):
            if str(newStateNumber).replace(" ","") not in TargetStates["obstacle"]:
                TargetStates["obstacle"].append(str(newStateNumber).replace(" ",""))

    #Victim Complete
    if numberOfAgents == 3:
        if (newStateNumber[2] == True) or (newStateNumber[5] == True) or (newStateNumber[8] == True):
            if str(newStateNumber).replace(" ","") not in TargetStates["victim"]:
                TargetStates["victim"].append(str(newStateNumber).replace(" ",""))
...
def isUMNumberViolated(newStateNumber, oldStateNumber, UMNumber, numberOfAgents, userQuery, actions):
    ...
    if newStateNumber != oldStateNumber:
        ...
        if numberOfAgents == 3:
            if ("True" in newStateNumberSplit[0] or "True" in newStateNumberSplit[3] or "True" in newStateNumberSplit[6])  and "fire" in tasksToDo:
                newUMNumber = float("-inf")
            if ("True" in newStateNumberSplit[1] or "True" in newStateNumberSplit[4] or "True" in newStateNumberSplit[7]) and "obstacle" in tasksToDo:
                newUMNumber = float("-inf")
            if ("True" in newStateNumberSplit[2] or "True" in newStateNumberSplit[5] or "True" in newStateNumberSplit[8]) and "victim" in tasksToDo:
                newUMNumber = float("-inf")
    else:
...
def main():
    ...
    numberOfAgents = 3
    numberOfTasks = 3
    task_locations = {"fire":(4,0), "obstacle":(3,4), "victim":(1,4)} #1
    ...
    #userQuery = {"agent1": ["*","fire","obstacle"],   #Possible Query
