...
#Orginal code location: https://github.com/uoe-agents/lb-foraging
...
class ForagingEnv(Env):
    ...
    def spawn_food(self, max_food, max_level,currentStart):
        food_count = 0
        attempts = 0
        min_level = max_level if self.force_coop else 1

        #3 Agents
        #self.field[3,4] = 2 #edit #obstacle
        #self.field[1,4] = 2 #edit #victim
        #self.field[4,0] = 2 #edit #fire


        if currentStart[0] != -1:
            self.field[currentStart[0],currentStart[1]] = currentStart[2]
        if currentStart[3] != -1:
            self.field[currentStart[3],currentStart[4]] = currentStart[5]
        if currentStart[6] != -1:
            self.field[currentStart[6],currentStart[7]] = currentStart[8]
    ...
    def spawn_players(self, max_player_level,currentState):
        #playerSpaces = [(0,3,1,"A",0),(0,4,1,"B",1),(0,5,1,"C",2)] # 3agents
        attempts = 0

        #Exp
        playerSpaces = [(currentState[9],currentState[10],1,"A",0),(currentState[12],currentState[13],1,"B",1),(currentState[15],currentState[16],1,"C",2)]

        for player in self.players:
            player.reward = 0
            row = playerSpaces[attempts][0]
            col = playerSpaces[attempts][1]
            player.playerNumber = playerSpaces[attempts][4]
            player.setup(
                    (row, col),
                    playerSpaces[attempts][2],
                    self.field_size,
                    )
            attempts += 1
    ...
    def _is_valid_action(self, player, action):
        ...
        currentPosition = (player.position[0],player.position[1])
        ...
        if action == Action.NONE:
            ...
            return (
                ...
                and currentPosition not in wallsNoNorth
            )
        elif action == Action.SOUTH:
            return (
                player.position[0] < self.rows - 1
                and self.field[player.position[0] + 1, player.position[1]] == 0
                and currentPosition not in wallsNoSouth
            )
        elif action == Action.WEST:
            return (
                player.position[1] > 0
                and self.field[player.position[0], player.position[1] - 1] == 0
                and currentPosition not in wallsNoWest
            )
        elif action == Action.EAST:
            return (
                player.position[1] < self.cols - 1
                and self.field[player.position[0], player.position[1] + 1] == 0
                and currentPosition not in wallsNoEast
            )
            ...
            if adjRow < player.position[0] and currentPosition in wallsNoNorth: #north
                notWallBlock = False
            elif adjRow > player.position[0] and currentPosition in wallsNoSouth: #south
                notWallBlock = False
            elif adjCol < player.position[1] and currentPosition in wallsNoWest: #west
                notWallBlock = False
            elif adjCol > player.position[1] and currentPosition in wallsNoEast: #east
                notWallBlock = False

            return (self.adjacent_food(*player.position) > 0 and notWallBlock)
    ...
    def reset(self):
        ...
        #3 Agents
        f = open("currentStartState.txt", "r")
        currentStart = f.read()
        currentStart = currentStart.replace("[","")
        currentStart = currentStart.replace("]","")
        currentStart = currentStart.replace(","," ")
        currentStart = [int(float(i)) for i in currentStart.split(" ") if i != '']
        #p#rint("reset",currentStart)
        ...
        self.spawn_players(self.max_player_level,currentStart)
        player_levels = sorted([player.level for player in self.players])
        self.spawn_food(
            self.max_food, 0,currentStart)
        self.current_step = 0
        self._game_over = False
        self._gen_valid_moves()

        failedFire = False
        f = open("failedFire.txt", "w")
        f.write(str(failedFire))
        f.close()
    ...
    def step(self, actions):
        ...
        for player, action in zip(self.players, actions):
            if action == Action.NONE:
                ...
                movements[player] = player.position
            elif action == Action.NORTH:
                collisions[(player.position[0] - 1, player.position[1])].append(player)
                collisions[player.position].append(player)
                movements[player] = (player.position[0] - 1, player.position[1])
            elif action == Action.SOUTH:
                collisions[(player.position[0] + 1, player.position[1])].append(player)
                collisions[player.position].append(player)
                movements[player] = (player.position[0] + 1, player.position[1])
                ...
                collisions[player.position].append(player)
                movements[player] = (player.position[0], player.position[1] - 1)
            elif action == Action.EAST:
                collisions[(player.position[0], player.position[1] + 1)].append(player)
                collisions[player.position].append(player)
                movements[player] = (player.position[0], player.position[1] + 1)
        ...
        for k, v in collisions.items():
             # Players can move to same space if two people don't move and one person can
            if len(v) > 1:
                for i in v:
                    movements.pop(i,-1)  # make sure no more than an player will arrive at location
                continue
        for i in movements.keys():
            i.position = movements[i]
        ...
        while loading_players:
            ...
            adj_player_numbers = ([a.playerNumber for a in adj_players])
            ...
            #3 Agents
            if(frow == 4 and fcol == 0): #Fire -> Any two agents
                if(player.playerNumber == 0 and (1 in adj_player_numbers and 2 in adj_player_numbers)) or (player.playerNumber == 1 and (0 in adj_player_numbers and 2 in adj_player_numbers)) or (player.playerNumber == 2 and (0 in adj_player_numbers and 1 in adj_player_numbers)):
                    continue

            if(frow == 3 and fcol == 4): #Obs -> Both Ground Robots
                if(player.playerNumber == 0 and 1 not in adj_player_numbers) or (player.playerNumber == 1 and 0 not in adj_player_numbers) or (2 in adj_player_numbers) or (player.playerNumber == 2):
                    continue

            if(frow == 1 and fcol == 4): #Victim -> 1 Ground and 1 Drone
                if(player.playerNumber == 0 and 2 not in adj_player_numbers) or (player.playerNumber == 1 and 2 not in adj_player_numbers) or (player.playerNumber == 2 and 0 not in adj_player_numbers and 1 not in adj_player_numbers) or (player.playerNumber == 0 and 1 in adj_player_numbers) or (player.playerNumber == 1 and 0 in adj_player_numbers) or (player.playerNumber == 2 and 0 in adj_player_numbers and 1 in adj_player_numbers):
                    continue

            # else the food was loaded and each player scores points
            for a in adj_players:
                if(frow == 4 and fcol == 0): #Fire Reward
                    #print("FIRE",adj_player_numbers)
                    a.reward = 10
                elif(frow == 3 and fcol == 4): #obs Reward
                    #print("OBSTACLE",adj_player_numbers)
                    a.reward = 10
                elif(frow == 1 and fcol == 4): #vic Reward
                    #print("VICTIM",adj_player_numbers)
                    a.reward = 10
