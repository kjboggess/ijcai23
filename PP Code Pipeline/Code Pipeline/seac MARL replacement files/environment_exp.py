...
class PressurePlate(gym.Env):
    ...
    def step(self, actions):
        ...
        for i, plate in enumerate(self.plates):
            if not plate.pressed:
                for i, agent in enumerate(self.agents):
                    if [plate.x, plate.y] == [self.agents[i].x, self.agents[i].y]:
                        plate.pressed = True
        ...
        if self.plates[0].pressed and self.plates[1].pressed:
            self.doors[0].open = True

        if self.plates[2].pressed and self.plates[3].pressed:
            self.doors[1].open = True

        if self.plates[4].pressed and self.plates[5].pressed:
            self.doors[2].open = True

        if self.plates[6].pressed and self.plates[7].pressed:
            self.doors[3].open = True
    ...
    def reset(self):
        # Grid wipe
        f = open("currentStartStatePP.txt", "r")
        currentStart = f.read()
        max_value = int(currentStart[-1])
        ...
        AgentPositions = [currentStart[0:102],currentStart[102:204],currentStart[204:306],currentStart[306:408],currentStart[408:510],currentStart[510:612],currentStart[612:714],currentStart[714:816],currentStart[816:918]]

        self.agent_order = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        # Agent
        self.agents = []
        for i in range(self.n_agents):
            self.agents.append(Agent(i,
                                    AgentPositions[self.agent_order[i]][0],
                                    AgentPositions[self.agent_order[i]][1]))
    ...
    def _get_rewards(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if (i == 0 or i == 1) and cur_room == 0:
                reward = - np.linalg.norm((np.array(plate_loc) - np.array(agent_loc)), 1) / self.max_dist
            elif (i == 2 or i == 3) and cur_room == 1:
                reward = - np.linalg.norm((np.array(plate_loc) - np.array(agent_loc)), 1) / self.max_dist
            elif (i == 4) and cur_room == 2:
                reward = - np.linalg.norm((np.array(plate_loc) - np.array(agent_loc)), 1) / self.max_dist
            else:
                reward = -2 + cur_room

            if agent.x == self.goal.x and agent.y == self.goal.y:
                reward = 10000000
