...
#Orginal code location: https://github.com/uoe-agents/robotic-warehouse
...
class Warehouse(gym.Env):
    ...
    def _make_obs(self, agent):
        ...
        if self.fast_obs:
            ...
            obs.write([agent.x, agent.y, int(agent.carrying_shelf is not None)]) #EDIT
            #shelf_id = self.grid[_LAYER_SHELFS, agent.y, agent.x]
            #obs.write([agent.x, agent.y, int(shelf_id)])
        ...
        obs["self"] = {
            ...
            #"carrying_shelf": [int(shelf_id)],
            "carrying_shelf": [int(agent.carrying_shelf is not None)], #EDIT
    ...
    def reset(self):
        f = open("holdingshelf.txt", "w")
        f.close()
        f = open("deliveredshelf.txt", "w")
        f.close()
        Shelf.counter = 0
        Agent.counter = 0
        ...
        #)
        self.request_queue = [self.shelfs[29],self.shelfs[10],self.shelfs[4],self.shelfs[14]]

        f = open("request_queue.txt", "w")
        for s in self.request_queue:
            f.write(str(s.x)+"-")
            f.write(str(s.y)+"-")
            shelf_id = self.grid[_LAYER_SHELFS, s.y, s.x]
            f.write(str(shelf_id)+"-")
        f.close()
    ...
    def step(
        ...
        #f = open("holdingshelf.txt", "w") #NOT NEEDED to reset here
        #f.close()
        #f = open("deliveredshelf.txt", "w")
        #f.close()
        ...
        for agent in self.agents:
            if agent.carrying_shelf:
                ...
                shelf_id = self.grid[_LAYER_SHELFS, agent.y, agent.x]
                f = open("holdingshelf.txt", "a")
                if (agent_id,shelf_id) != (0,0):
                    f.write(str((agent_id,shelf_id)))
                    self.pickedupShelfs.append((agent_id,shelf_id))
                f.close()
        ...
        for x, y in self.goals:
            ...
            f = open("deliveredshelf.txt", "a")
            agent_id = self.grid[_LAYER_AGENTS, y, x]
            f.write(str((agent_id,shelf_id)))
            self.deliveredShelfs.append((agent_id,shelf_id))
            f.close()
            ...
            #shelf_id = self.grid[_LAYER_SHELFS, new_request.y, new_request.x]
        ...
        if (self.max_inactivity_steps and self._cur_inactive_steps >= self.max_inactivity_steps) or (self.max_steps and self._cur_steps >= self.max_steps) or len(self.request_queue) == 0:
        ...
        for s in self.request_queue:
            shelf_id = self.grid[_LAYER_SHELFS, s.y, s.x]
            QUEUE.append(shelf_id)

        self.delivered[str(list(new_obs[0])+list(new_obs[1]))] = str(self.deliveredShelfs)
        self.pickedup[str(list(new_obs[0])+list(new_obs[1]))] = str(self.pickedupShelfs)

        #self.delivered[str(list(new_obs[0])+list(new_obs[1])+list(new_obs[2]))] = str(self.deliveredShelfs)
        #self.pickedup[str(list(new_obs[0])+list(new_obs[1])+list(new_obs[2]))] = str(self.pickedupShelfs)

        #self.delivered[str(list(new_obs[0])+list(new_obs[1])+list(new_obs[2])+list(new_obs[3]))] = str(self.deliveredShelfs)
        #self.pickedup[str(list(new_obs[0])+list(new_obs[1])+list(new_obs[2])+list(new_obs[3]))] = str(self.pickedupShelfs)

        json.dump(self.pickedup, open("RWARE_stateToQPickUp.txt",'w'))
        json.dump(self.delivered, open("RWARE_stateToQDeliver.txt",'w'))
        #f = open("RWARE_stateToQ.txt", "w")
        #f.write("pickedUp = ")
        #f.write(str(self.pickedup))
        #f.write("\n")
        #f.write("delivered = ")
        #f.write(str(self.delivered))
        #f.close()
