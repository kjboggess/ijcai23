...
class Policy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        ...
        #if deterministic: #For train and evaluate
            #action = dist.mode() #For train and evaluate
        #else: #For train and evaluate
            #action = dist.sample() #For train and evaluate

        action, newProbs = dist.sample() #For exp

        newProbsIndex = [i for i in range(0,len(newProbs)) if newProbs[i].item() >= 1] #For exp

        action_log_probs = dist.log_probs(action)
        dist_entropy = dist.entropy().mean()

        return value, action, action_log_probs, rnn_hxs, newProbsIndex #For exp
        #return value, action, action_log_probs, rnn_hxs #for train and evaluate
