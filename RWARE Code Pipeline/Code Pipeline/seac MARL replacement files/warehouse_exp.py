...
class Warehouse(gym.Env):
    ...
    def reset(self):
        ...
        f = open("currentStartStateRWARE.txt", "r")
        wholeFile = f.read()
        startpoint = wholeFile.find("*")
        currentStart = wholeFile[:startpoint]
        currentStart = currentStart.replace("[","")
        currentStart = currentStart.replace("]","")
        currentStart = currentStart.replace(","," ")
        currentStart = [int(float(i)) for i in currentStart.split(" ") if i != '']
        newStateNumber = wholeFile[startpoint+1:]
        newStateNumber = newStateNumber.replace("[","")
        newStateNumber = newStateNumber.replace("]","")
        newStateNumber = newStateNumber.replace(","," ")
        newStateNumber = [i for i in newStateNumber.split(" ") if i != '']
        ...
        for i in range(self.n_agents):
            location = currentStart[1+(119*i)]*10
            location = location + currentStart[0+(119*i)]
            agent_locs.append(location)
        ...
        for i in range(22,30):
            if "False" in newStateNumber[i] and self.shelfs[shelfsLIST[(i+2)//2-12]] not in self.request_queue:
                self.request_queue.append(self.shelfs[shelfsLIST[(i+2)//2-12]])
